defmodule PackageCompareTest do
  ...
  @test_json_without_dev_dependencies """
  {
    "name": "ghpr",
    "version": "1.0.0",
    "description": "A dashboard for a github team's pull requests",
  ...
  test "parse json without dev dependencies" do
    result = PackageCompare.parse(@test_json_without_dev_dependencies)
    assert Map.has_key?(result, :name)
    assert Map.has_key?(result, :version)
    refute Map.has_key?(result, :devDependencies)
    assert Map.has_key?(result, :dependencies)

    assert Map.keys(result) == [:dependencies, :name, :version]
  end

  test "add_commands from json without dev dependencies" do
    result = PackageCompare.parse(@test_json_without_dev_dependencies)

    commands = PackageCompare.add_package(result)

    assert commands.nodes |> Enum.count() == 14
    assert commands.relationships |> Enum.count() == 13
  end
end
